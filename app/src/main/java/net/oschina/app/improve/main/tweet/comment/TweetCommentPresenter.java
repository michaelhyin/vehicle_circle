...
class TweetCommentPresenter implements TweetCommentContract.Presenter {
    ...
    public void onRefreshing() {
        if (mTweet == null)
            return;
        LeanCloudApi.getTweetCommentList(mTweet.getId(), "", new TextHttpResponseHandler() {
            @Override
    ...
    public void onLoadMore() {
        if (mTweet == null)
            return;
        LeanCloudApi.getTweetCommentList(mTweet.getId(), mNextToken, new TextHttpResponseHandler() {
            @Override
            public void onFailure(int statusCode, Header[] headers, String responseString, Throwable throwable) {
                mView.showNetworkError(R.string.network_timeout_hint);
                mView.onComplete();
