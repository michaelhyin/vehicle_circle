 ...
 * on 16/8/26.
 */
public class TweetDetailPicturesLayout extends ViewGroup implements View.OnClickListener {
    ...
    public TweetDetailPicturesLayout(Context context) {
        this(context, null);
    }

    public TweetDetailPicturesLayout(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public TweetDetailPicturesLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        init(attrs, defStyleAttr, 0);
    }

    @RequiresApi(api = Build.VERSION_CODES.LOLLIPOP)
    public TweetDetailPicturesLayout(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
    ...
    private void init(AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        if (attrs != null) {
            ...
            setColumn(a.getInt(R.styleable.TweetPicturesLayout_column, 2));
...
//                    if (i<=4)
        ...
        if (mImages != null && mImages.length > 0) {
            ...
            for (int i = 0; i < mImages.length; i++) {
                ...
                builder = requestManager.load(path)
                        //.bitmapTransform(new CropTransformation(getContext(), 600, 200, CropTransformation.CropType.CENTER))
//                        .centerCrop()
...
//        } else {
            ...
            // Get child size
//            int childSize = getMaxChildSize((int) (maxContentWidth / mColumn));
            int childSize = getMaxChildSize((int) (maxContentWidth + mHorizontalSpacing));


            int width = childSize;
            int height = childSize;
            for (int i = 0; i < childCount; ++i) {
                ...
                if (Tweet.Image.check(image)) {
                    int imageW = image.getW();
                    int imageH = image.getH();
                    height = (int) (imageH / ((float) imageW / width));

//                if ((childCount == 3 && i == 2) || (childCount == 1 && i == 0))
//                {
//                    width = getMaxChildSize((int) (maxContentWidth + mHorizontalSpacing));
//                    if (i == 0)
//                    {
//                        height = width;
//                        childSize = width;
...
//                }
                    ...
                    if (i != (childCount - 1))
                    {
                        wantedHeight += mVerticalSpacing;
                    }
                }

            }

//            int lines = (int) (childCount / (float) mColumn);
        ...
        if (childCount > 0) {
            ...
            if (childCount == 1) {
                ...
                for (int i = 0; i < childCount; ++i) {
                    ...
                    if (childLeft + childWidth + paddingRight > mWidth && i != 0) {
